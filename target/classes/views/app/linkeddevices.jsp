...
<div class="settings-container">
    ...
    <div class="account-section">
        ...
        <!-- Device List -->
        <%
            List<UserSession> devices = (List<UserSession>) request.getAttribute("devices");
            String currentSessionId = (String) request.getAttribute("currentSessionId");
        ...
        <div class="device-item">
            <div class="device-info">
                <div class="device-name"><%= device.getDeviceName() %></div>
                <div class="device-details"><%= device.getDeviceType() %>, Last login: <%= lastLogin %></div>
            </div>
            <% if (!isCurrentDevice) { %>
            <form method="post" action="${pageContext.request.contextPath}/linkeddevicesservlet" class="remove-form">
                <input type="hidden" name="action" value="removeDevice">
                <input type="hidden" name="sessionId" value="<%= device.getSessionId() %>">
                <button type="submit" class="remove-btn" onclick="return confirm('Remove this device?')">×</button>
            </form>
            <% } else { %>
        ...
        <form method="post" action="${pageContext.request.contextPath}/linkeddevicesservlet">
            <input type="hidden" name="action" value="logoutAll">
            <button type="submit" class="logout-all-btn" onclick="return confirm('Logout from all other devices?')">Logout from all devices</button>
        </form>
    </div>
</div>
<script>
