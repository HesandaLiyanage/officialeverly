...
<body>
            ...
            <div class="post-image-section">
                <div class="carousel-container" data-current-slide="0">
                    <div class="carousel-track">
                        <c:choose>
                            ...
                            <c:when test="${not empty post.coverMediaUrl}">
                                <div class="carousel-slide">
                                    <img src="${fn:escapeXml(post.coverMediaUrl)}" alt="Post image">
                                </div>
                            </c:when>
                            <c:otherwise>
                                <div class="carousel-slide" style="background: ${ownerGradient};">
                    ...
                    <c:if test="${hasMultipleMedia}">
                        <button class="carousel-btn prev" type="button" onclick="moveCarousel(-1)">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                <polyline points="15 18 9 12 15 6"></polyline>
                            </svg>
                        </button>
                        <button class="carousel-btn next" type="button" onclick="moveCarousel(1)">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    ...
                    <div class="user-info">
                        <c:choose>
                            <c:when test="${hasOwnerPic}">
                                <img src="${fn:escapeXml(ownerPic)}"
                                    alt="${fn:escapeXml(post.feedProfile.feedUsername)}"
                                    class="user-avatar"
                                    style="width:40px;height:40px;object-fit:cover">
                            </c:when>
                            <c:otherwise>
                                <div class="user-avatar" style="background:${ownerGradient};width:40px;height:40px;">
                                    <span>${fn:escapeXml(post.feedProfile.initials)}</span>
                                </div>
                            </c:otherwise>
                        </c:choose>
                        <div class="user-details">
                            <h4 class="username">${fn:escapeXml(post.feedProfile.feedUsername)}</h4>
                            <c:if test="${not empty post.memory && not empty post.memory.title}">
                                <p class="post-location">${fn:escapeXml(post.memory.title)}</p>
                            </c:if>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="comment-item original-caption">
                            ...
                            <div class="comment-content">
                                <div class="comment-header">
                                    <span class="comment-username">${fn:escapeXml(post.feedProfile.feedUsername)}</span>
                                    <span class="comment-time">${post.relativeTime}</span>
                                </div>
                                <p class="comment-text">${fn:escapeXml(post.caption)}</p>
                            </div>
                        </div>
                    </c:if>
                    ...
                    <c:choose>
                        <c:when test="${empty comments}">
                            <div class="no-comments">
                                ...
                                <p class="no-comments-subtitle">Start the conversation.</p>
                            </div>
                        </c:when>
                        <c:otherwise>
                            <c:forEach items="${comments}" var="comment">
                                ...
                                <c:if test="${empty comment.parentCommentId}">
                                    ...
                                    <c:set var="commenterPic" value="${not empty commenter ? commenter.feedProfilePictureUrl : ''}" />
                                    <c:set var="hasCommenterPic" value="${not empty commenterPic && !fn:contains(commenterPic, 'default')}" />
                                    <c:set var="commenterInitials" value="${not empty commenter && not empty commenter.initials ? commenter.initials : '?'}" />
                                    <c:set var="commenterUsername" value="${not empty commenter && not empty commenter.feedUsername ? commenter.feedUsername : 'Unknown'}" />
                                    <c:set var="commentLiked" value="${comment.likedByCurrentUser}" />
                                    <c:set var="cLikeCount" value="${comment.likeCount}" />
                                    <c:set var="canDelete" value="${isPostOwner || (not empty currentProfile && comment.feedProfileId == currentProfile.feedProfileId)}" />
                                    <c:set var="likedClass" value="${commentLiked ? ' liked' : ''}" />
                                    <c:set var="fillCol" value="${commentLiked ? '#ed4956' : 'none'}" />
                                    <c:set var="strokeCol" value="${commentLiked ? '#ed4956' : 'currentColor'}" />

                                    <div class="comment-item" data-comment-id="${comment.commentId}">
                                        <c:choose>
                                            <c:when test="${hasCommenterPic}">
                                                <img src="${fn:escapeXml(commenterPic)}"
                                                    alt="${fn:escapeXml(commenterUsername)}"
                                                    class="comment-avatar">
                                            </c:when>
                                            <c:otherwise>
                                                <div class="comment-avatar" style="background:${ownerGradient}">
                                                    <span>${fn:escapeXml(commenterInitials)}</span>
                                                </div>
                                            </c:otherwise>
                                        </c:choose>
                                        <div class="comment-content">
                                            <div class="comment-header">
                                                <span class="comment-username">${fn:escapeXml(commenterUsername)}</span>
                                                <span class="comment-time">${comment.relativeTime}</span>
                                            </div>
                                            <p class="comment-text">${fn:escapeXml(comment.commentText)}</p>
                                            <div class="comment-actions">
                                                ...
                                                <c:if test="${cLikeCount > 0}">
                                                    <span class="comment-likes">${cLikeCount} likes</span>
                                                </c:if>
                                                <button class="reply-btn"
                                                    data-username="${fn:escapeXml(commenterUsername)}"
    ...
    <script>
        ...
        var currentUserInitials = '${fn:escapeXml(not empty currentProfile ? currentProfile.initials : "")}';
        var currentUsername = '${fn:escapeXml(not empty currentProfile ? currentProfile.feedUsername : "")}';
        var currentProfilePicUrl = '${fn:escapeXml(cpUrlSafe)}';
