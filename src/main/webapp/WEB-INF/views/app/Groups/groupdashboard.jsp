                    ...
                    <div class="groups-list" id="groupsList"
                      ...
                      <% List<Group> groups = (List<Group>) request.getAttribute("groups");
                          ...
                          <% } else { for (Group group : groups) { // Calculate time ago java.sql.Timestamp
                            ...
                            diffInHours=TimeUnit.HOURS.convert(diffInMillies, TimeUnit.MILLISECONDS); if
                            (diffInHours==0) { long diffInMinutes=TimeUnit.MINUTES.convert(diffInMillies,
                            TimeUnit.MILLISECONDS); timeAgo=diffInMinutes + " minutes ago" ; } else {
                            timeAgo=diffInHours + " hours ago" ; } } else if (diffInDays==1) { timeAgo="Yesterday" ; }
                            else if (diffInDays < 7) { timeAgo=diffInDays + " days ago" ; } else if (diffInDays < 30) {
                            long weeks=diffInDays / 7; timeAgo=weeks + " week" + (weeks> 1 ? "s" : "") + " ago";
                            } else if (diffInDays < 365) { long months=diffInDays / 30; timeAgo=months + " month" +
                              (months> 1 ? "s" : "") + " ago";
                              } else {
                              ...
                              <a href="<%= request.getContextPath() %>/groupmemories?groupId=<%= group.getGroupId() %>"
                                class="group-card" data-group-title="<%= group.getName().toLowerCase() %>">
                                <div class="group-images single">
                                  <% if (group.getGroupPicUrl() !=null && !group.getGroupPicUrl().isEmpty()) { %>
                                    <div class="image-card"
                                      style="background-image: url('<%= group.getGroupPicUrl() %>'); background-size: cover; background-position: center;">
                                    </div>
                                    <% } else { %>
                    ...
                    <div class="sidebar-section">
                      ...
                      <div class="announcements-list">
                        <% // Display recent groups as announcements if (groups !=null && !groups.isEmpty()) { int
                          ...
                          timeAgo = "1 day ago";
                          } else if (diffInDays < 7) { timeAgo=diffInDays + " days ago" ; } else { long weeks=diffInDays
                            / 7; timeAgo=weeks + " week" + (weeks> 1 ? "s" : "") + " ago";
                            }
                            %>
                              ...
                              <div class="announcement-content">
                                ...
                                <p class="announcement-text">
                                  <%= group.getDescription() !=null && !group.getDescription().isEmpty() ?
                                    group.getDescription() : "New group created!" %>
                                </p>
