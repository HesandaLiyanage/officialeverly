...
<head>
    ...
    <title>${fn:escapeXml(announcement.title)} - Everly</title>
    <link
...
<body>
    <div class="mv-page">
        <!-- Close button -->
        <a href="${pageContext.request.contextPath}/groupannouncement?groupId=${announcement.groupId}"
            class="mv-close-btn">
        ...
        <div class="mv-wrapper">
            ...
            <div class="mv-media-section">
                <c:choose>
                    <c:when test="${hasEvent and not empty eventPicUrl}">
            ...
            <div class="mv-details-section">
                ...
                <div class="mv-header">
                    <div class="mv-header-left">
                        <div class="mv-avatar">
                            <c:out value="${authorInitial}" />
                        </div>
                        <div class="mv-header-text">
                            <h3><c:out value="${authorName}" /></h3>
                ...
                <div class="mv-content">
                    ...
                    <c:if test="${hasEvent and not empty eventPicUrl}">
                        <div class="mv-info-group">
                    ...
                    <!-- Linked Event Info -->
                    <c:if test="${hasEvent and not empty linkedEventTitle}">
                        <div class="mv-divider"></div>
                        <div class="mv-info-group">
                            <div class="mv-info-label">Event</div>
                            <div class="mv-info-value" style="font-weight: 600;">
                                <c:out value="${linkedEventTitle}" />
                            </div>
                            <div class="mv-info-value date" style="margin-top: 4px;">
                                <svg viewBox="0 0 24 24" fill="none"
                                    stroke="currentColor" stroke-width="2">
                            ...
                            <a href="${pageContext.request.contextPath}/eventinfo?id=${announcement.eventId}"
                                class="ev-event-link">
                                <svg width="10" height="10" viewBox="0 0 24 24"
                                    fill="none" stroke="currentColor"
                                    stroke-width="2.5">
                        ...
                        <div class="mv-info-group">
                            ...
                            <div class="ev-poll-card">
                                <div class="ev-poll-question">Are you going?</div>
                                <p class="ev-poll-hint">Tap to vote. Tap again to
                                    un-vote.</p>

                                <div class="ev-poll-options">
                                    <button type="button"
                                        class="ev-poll-btn ${userCurrentVote eq 'going' ? 'selected going' : ''}"
                                        onclick="castVote(${pollEventId}, ${pollGroupId}, 'going', this)">
                                        <span class="ev-poll-btn-left"><span
                                                class="ev-poll-btn-icon">✅</span>Going</span>
                                        <span class="ev-poll-count"
                                            id="count-going-${pollGroupId}">${goingCount}</span>
                                    </button>
                                    <div class="ev-poll-bar-wrap">
                                        <div class="ev-poll-bar going"
                                            id="bar-going-${pollGroupId}"
                                            style="width:${goingPercent}%"></div>
                                    </div>

                                    <button type="button"
                                        class="ev-poll-btn ${userCurrentVote eq 'not_going' ? 'selected not_going' : ''}"
                                        onclick="castVote(${pollEventId}, ${pollGroupId}, 'not_going', this)">
                                        <span class="ev-poll-btn-left"><span
                                                class="ev-poll-btn-icon">❌</span>Not
                                            Going</span>
                                        <span class="ev-poll-count"
                                            id="count-not_going-${pollGroupId}">${notGoingCount}</span>
                                    </button>
                                    <div class="ev-poll-bar-wrap">
                                        <div class="ev-poll-bar not_going"
                                            id="bar-not_going-${pollGroupId}"
                                            style="width:${notGoingPercent}%">
                                        </div>
                                    </div>

                                    <button type="button"
                                        class="ev-poll-btn ${userCurrentVote eq 'maybe' ? 'selected maybe' : ''}"
                                        onclick="castVote(${pollEventId}, ${pollGroupId}, 'maybe', this)">
                                        <span class="ev-poll-btn-left"><span
                                                class="ev-poll-btn-icon">🤔</span>Maybe</span>
                                        <span class="ev-poll-count"
                                            id="count-maybe-${pollGroupId}">${maybeCount}</span>
                                    </button>
                                    <div class="ev-poll-bar-wrap">
                                ...
                                <p class="ev-poll-total"
                                    ...
                                    <c:out value="${totalVotesLabel}" />
                                </p>

                                <button type="button"
                                    class="ev-poll-voters-toggle"
                                    onclick="toggleVoters(${pollGroupId})">Show who
                                    voted</button>
                                <div class="ev-poll-voters-list"
                                    id="voters-list-${pollGroupId}">
                                    <c:choose>
                                        <c:when
                                            test="${not empty voterDisplayData}">
                                            <c:forEach var="voter"
                                                items="${voterDisplayData}">
                                                <div class="ev-poll-voter">
                                                    ...
                                                    <span
                                                        class="ev-poll-voter-name">${fn:escapeXml(voter.name)}</span>
                                                    <span
                                                        class="ev-poll-voter-badge ${voter.vote}">${voter.label}</span>
                                                </div>
                                            </c:forEach>
                                        </c:when>
                                        <c:otherwise>
